...
'formats': ('U30', 'str', 'str', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 
...
ticid, name,kelt_p,age,kelt_m,kelt_m_err,kelt_r,kelt_r_err,kelt_teff,kelt_teff_err,notes = np.loadtxt(kelpebfile,unpack=True, dtype=dtyp_dict, delimiter='\t')#np.genfromtxt(ebfile,dtype=dtyp_dict,unpack=True,comments="#")
...
#	kelt_rinf_err[i] = math.sqrt(pow( kelt_r_err[i]/kelt_r_mod[i],2.) + pow(kelt_r[i]*kelt_r_mod_err[i]/pow(kelt_r_mod[i],2.),2.))

#	kelt_teff_mod[i] = np.array(teff_interp_func(kelt_m[i]))
#	kelt_teff_mod_err[i] = np.std(teff_interp_func(kelt_mass_draws))
#	kelt_teffinf[i] = kelt_teff[i]/kelt_teff_mod[i]
#	kelt_teffinf_err[i] = math.sqrt(pow(kelt_teff_err[i]/kelt_teff_mod[i],2.) + pow(kelt_teff[i]*kelt_teff_mod_err[i]/pow(kelt_teff_mod[i],2.),2.))
...
#	eb_rinf_err[i] = math.sqrt(pow(eb_r_err[i]/eb_r_mod[i],2.) +pow(eb_r[i]*eb_r_mod_err[i]/pow(eb_r_mod[i],2.),2.))
	
#	eb_teff_mod[i] = massteff_interpolator_1g(eb_m[i])	
#	mti_draws = massteff_interpolator_1g(eb_mass_draws)
#	eb_teff_mod_err[i] = np.std(mti_draws)
#	eb_teffinf[i] = eb_teff[i]/eb_teff_mod[i]
#	eb_teffinf_err[i] = math.sqrt(pow(eb_teff_err[i]/eb_teff_mod[i],2.) + pow(eb_teff[i]*eb_teff_mod_err[i]/pow(eb_teff_mod[i],2.),2.))
	
#	eb_l_mod[i] = eb_r_mod[i]**2. * (eb_teff_mod[i]/teffsun)**4.
#	eb_l_mod_err[i] = np.std([mri_draws[i]**2. * (mti_draws[i]/teffsun)**4. for i in range(len(mri_draws))])
#	eb_linf[i] = eb_l[i] / eb_l_mod[i]
	
#	eb_linf_err[i] = eb_linf[i]*((eb_l_mod_err[i]/eb_l_mod[i])**2. + (eb_l_err[i]/eb_l[i])**2.)
	#eb_linf_err[i] = np.sqrt((eb_l_err[i] / eb_l_mod[i])**2. + (eb_l[i] * eb_l_mod_err[i] / eb_l_mod[i]**2.)**2.)
...
print("KELT Radius inflation:",kelt_rinf)
print('KELP M-dwarf radius inflation error:',kelt_rinf_err)

print("KELT M-dwarf model Teff error:", kelt_teff_mod_err)
print("KELT Teff suppression:",kelt_teffinf)
print("KELP M-dwarf Teff suppression error:",kelt_teffinf_err)
...
#plt.plot(np.sort(eb_p),radline)
#plt.fill_between(np.sort(eb_p),lower3sig_radline, upper3sig_radline, color='b',alpha=0.25)
#plt.fill_between(np.sort(eb_p),lower_radline, upper_radline, color='b',alpha=0.25)

#plt.errorbar(kelt_p[0], kelt_rinf[0], color='b', linestyle='',yerr=kelt_rinf_err[0], zorder=100)
#plt.scatter(kelt_p[0], kelt_rinf[0], color='b', label=r'${\rm K16912\ B\ (SED)}$', marker='o', zorder=100)#, label=r'${\rm our\ sample}$',marker='o',zorder=20)#,s=5)
#plt.errorbar(kelt_p[1],kelt_rinf[1],yerr=kelt_rinf_err[1],linestyle='',color='b',zorder=15)
#plt.scatter(kelt_p[1],kelt_rinf[1],color='b',label=r'${\rm K16912\ B\ (MIST\ iso.)}$',zorder=15)#,s=5)
plt.errorbar(kelt_p,kelt_rinf,color='r',linestyle='',yerr=kelt_rinf_err,zorder=20)
plt.scatter(kelt_p,kelt_rinf,color='r',label=r'${\rm SLEB\ sample}$',marker='o',zorder=20)#,s=5)
#plt.errorbar(kelt_p[2],kelt_rinf[2],yerr=kelt_rinf_err[2],linestyle='',color='black')
#plt.errorbar(kelt_p[2],kelt_rinf[2],yerr=kelt_rinf_err[2],linestyle='',color='black')
#plt.scatter(kelt_p[2],kelt_rinf[2],color='black',label=r'${\rm Combined}$',marker='*')
#plt.errorbar(tessp,tessinf,yerr=tessrerr2,linestyle='',color='r')
#plt.scatter(tessp,tessinf,color='r',label='TESS M Dwarfs')
...
#plt.errorbar(kelt_p[0], kelt_teffinf[0], color='b', linestyle='',yerr=kelt_teffinf_err[0], zorder=100)
#plt.scatter(kelt_p[0], kelt_teffinf[0], color='b', label=r'${\rm K16912\ B\ (SED)}$', marker='o', zorder=100)#, label=r'${\rm our\ sample}$',marker='o',zorder=20)#,s=5)
#plt.errorbar(kelt_p[1],kelt_teffinf[1],yerr=kelt_teffinf_err[1],linestyle='',color='b',zorder=15)
#plt.scatter(kelt_p[1],kelt_teffinf[1],color='b',label=r'${\rm K16912\ B\ (MIST\ iso.)}$',zorder=15)#,s=5)
#plt.plot(np.sort(eb_p),teffline)
#plt.fill_between(np.sort(eb_p),lower3sig_teffline, upper3sig_teffline, color='b',alpha=0.25)
#plt.fill_between(np.sort(eb_p),lower_teffline, upper_teffline, color='b',alpha=0.25)
plt.errorbar(kelt_p,kelt_teffinf,yerr=kelt_teffinf_err,linestyle='',color='r',zorder=20)
plt.scatter(kelt_p,kelt_teffinf,color='r',label=r'${\rm SLEB\ sample}$',zorder=20)
#plt.errorbar(kelt_p[1],kelt_teffinf[1],yerr=kelt_teffinf_err[1],linestyle='',color='b',zorder=20)
#plt.scatter(kelt_p[1],kelt_teffinf[1],color='b',label=r'${\rm With-MIST\ Fit}$',zorder=20)
plt.axhline(1.0,linestyle='--',color='k')
plt.xlim([0.1,100])
plt.legend(loc='best')
plt.xlabel(r'$\rm Period\ (days)$',fontsize=16)
...
#plt.errorbar(kelt_p[1],kelt_teffinf[1],yerr=kelt_teffinf_err[1],linestyle='',color='b',zorder=15)
#plt.scatter(kelt_p[1],kelt_teffinf[1],color='b',label=r'${\rm HD\ 74925\ B\ (MIST\ iso.)}$',zorder=15)#,s=5)
#plt.errorbar(kelt_p[1],kelt_teffinf[1],yerr=kelt_teffinf_err[1],linestyle='',color='b',zorder=20)
#plt.scatter(kelt_p[1],kelt_teffinf[1],color='b',label=r'${\rm With-MIST\ Fit}$',zorder=20)
plt.axhline(1.0,linestyle='--',color='k')
plt.xlim([0.1,100])
